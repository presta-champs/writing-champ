...
export default function NewArticlePageWrapper() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-[50vh]"><div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderColor: "var(--accent)" }} /></div>}>
      <NewArticlePage />
    </Suspense>
  );
}
...
function NewArticlePage() {
  ...
  // Pre-fill brief from duplicated article
  useEffect(() => {
    if (!duplicateId || duplicateLoaded) return;
    async function loadDuplicate() {
      const result = await duplicateArticle(duplicateId!);
      if (!result.data) return;
      const d = result.data;

      setBrief((prev) => ({
        ...prev,
        topic: d.title || prev.topic,
        format: d.format || prev.format,
        primaryKeyword: d.primaryKeyword || prev.primaryKeyword,
        secondaryKeywords: d.secondaryKeywords
          ? d.secondaryKeywords.join(", ")
          : prev.secondaryKeywords,
        readabilityTarget: d.readabilityTarget ?? prev.readabilityTarget,
      }));

      // Pre-select website and persona after lists have loaded —
      // store IDs and let the website/persona load effects match them.
      if (d.websiteId) {
        ...
        if (ws) {
          ...
          // Also pre-select persona if available
          if (d.personaId) {
