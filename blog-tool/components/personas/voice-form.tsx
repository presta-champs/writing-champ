...
/** Safely coerce a value that might be a string or array into a string[] */
...
/** Safely coerce a value that might not be an array into T[] */
...
export function PersonaVoiceForm({ persona }: { persona: Persona }) {
    ...
    const [voicePrinciples, setVoicePrinciples] = useState<{ title: string; description: string }[]>(toArray(persona.voice_principles))
    const [sentenceRulesDo, setSentenceRulesDo] = useState<string[]>(toStringArray(persona.sentence_rules_do))
    const [sentenceRulesDont, setSentenceRulesDont] = useState<string[]>(toStringArray(persona.sentence_rules_dont))
    const [structuralPatterns, setStructuralPatterns] = useState<{ title: string; description: string }[]>(
        toArray<{ name: string; description: string }>(persona.structural_patterns).map(p => ({ title: p.name, description: p.description }))
    )
    const [recurringThemes, setRecurringThemes] = useState<string[]>(toStringArray(persona.recurring_themes))
    const [examplePassages, setExamplePassages] = useState<{ title: string; topic: string; text: string }[]>(toArray(persona.example_passages))
    const [badges, setBadges] = useState<string[]>(toStringArray(persona.badges))
